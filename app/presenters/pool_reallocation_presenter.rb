...
# single-row twin of the distribution screen.
#
# ── THE POOL-ERA TWIN, AND TASK 6 DELETES THIS WHOLE FILE. `ReallocationPresenter` is the same
# screen re-anchored on the purpose ledger (two-ledger spec §2): categories hold the money, so a
# move is `category → category` or `available ↔ category` and nothing crosses anything. This copy
# survives Task 4 for ONE caller — `HomePresenter#damage_reader`, whose rows are still pools until
# Task 6 — and for the `/pool_movements/new` screen those fix buttons link to. It was renamed
# rather than kept under the canonical name so that Task 6's work is a deletion rather than a
# rename. Nothing here changed with the cutover; do not fix bugs in it, fix them in the twin. READ-ONLY: this presenter writes nothing, and it
...
class PoolReallocationPresenter
