...
RSpec.describe "Allocations New Damage", type: :system do
  ...
  describe "moving $300 into Dentist", :aggregate_failures do
    ...
    it "names the rule that slips and what the category will ask for" do
    ...
    # THE OPPOSITE DIRECTION, on the same screen. A savings goal with no rules holds nothing back, so
    # the move takes only free money: nothing slips and nothing asks for more, and the line says so
    # by not saying it (spec §5: state the consequence only when there is one).
    ...
    # THE ROOT SAYS LESS THAN ANY CATEGORY, by construction rather than by a branch in the copy:
    # available holds money for no rule, so nothing can slip and no ask can move, and the balance
    # arrow is the whole of what there is to say. Grey, which is the third colour below.
    it "says only the arrow when the money comes out of the root" do
      within("[data-damage='Available']") do
        expect(page).to have_content("$810.00 → $510.00")
        expect(page).to have_no_content("slips to")
        expect(page).to have_no_content("a period instead of")
        expect(page).to have_no_content("becomes")
      end
    end
    ...
    # THREE COLOURS FOR THREE DIFFERENT THINGS, mutually exclusive: a move inside `free_amount` costs
    # nothing and is stated in grey, a move that reaches PAST it into money a rule was holding is
    # amber, and one that pushes the category into a new state is red.
    #
    # The two negatives are what make it bite — without them a version that painted everything grey,
    # or everything amber, would still pass.
    it "colours a move by whether it takes money a rule was holding" do
      ...
      expect(page).to have_css("[data-damage='Available'].text-gray-500")
  ...
  # THE OTHER DESTINATION. A withdrawal back to available is a real move (spec §3: withdrawing from
  # savings is category → available), and the root has no status — so the gain sentence is the arrow
  # and nothing else, where a category's carries its state.
  describe "moving $200 out of Cushion and back to available", :aggregate_failures do
    before { visit new_allocation_path(to_category_id: ReallocationPresenter::ROOT.id, amount: 200) }

    it "states the arrival with no state to report" do
      within("#reallocation-gain") do
        expect(page).to have_content("Available $810.00 → $1,010.00")
        expect(page).to have_no_content("becomes")
      end
      expect(page).to have_no_css("[data-source='Available']")
  ...
  # the next distribution takes its whole leftover back and tops the category up to its full rate
  ...
  describe "moving $100 out of a category whose period has closed", :aggregate_failures do
    before { visit new_allocation_path(to_category_id: dentist.id, amount: 100) }
