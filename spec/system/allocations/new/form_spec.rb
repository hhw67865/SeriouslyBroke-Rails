...
RSpec.describe "Allocations New Form", type: :system do
  ...
  describe "arriving and recomputing", :aggregate_failures do
    # THE LINK SHAPE HOME WILL USE once its rows are categories, arriving with both ends and the
    # amount already chosen: the boxes come back holding them and the move is one click away.
    ...
    # THE ROOT ROUND-TRIPS BY NAME, not by a uuid: `"available"` is what the select and the radio
    # both carry, and it is the one value on this screen that cannot collide with a category id.
    it "arrives prefilled with the root on either end" do
      visit new_allocation_path(
        to_category_id: dentist.id, from_category_id: ReallocationPresenter::ROOT.id, amount: 300
      )
    ...
    # MEASURED IN A BROWSER, NOT REASONED ABOUT. The page carries three query parameters, and
    # `shared/_date_selector` re-emitted every one of them as a hidden field — with an id — in each of
    # its forms. `label for=` and `getElementById` both reach the first match in the document, so
    # `document.getElementById("amount")` here returned `<input type="hidden" name="amount"
    # value="300">` rather than the box the user types in. Every other example passed either way,
    # because Capybara filters invisible elements and landed on the right one by luck — which is why
    # this is asserted directly rather than through `fill_in`.
    ...
    # distribution screen shipped exactly that defect and it wrote a $2,900 split on a keypress. Both
    # halves on one keystroke: nothing was written, and the recompute that should have happened did.
    it "recomputes rather than moving money when Enter is pressed in the amount box" do
      ...
      expect(Allocation.where(from_category: car, to_category: dentist)).not_to exist
