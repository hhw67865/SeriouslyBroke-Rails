...
RSpec.describe "Entries Index - Search", type: :system do
  ...
  before do
    ...
    # These two were the categories pointing at the two savings GOALS, and they are ordinary
    # categories now (two-ledger spec §5, Task 8) — the entries on them are still here because the
    # date, description and category searches below count them.
  ...
  describe "search form display", :aggregate_failures do
    ...
    it "shows all search field options" do
      # "Pool" WAS THE FIFTH OPTION AND IS GONE WITH THE COLUMN (two-ledger spec §5, Task 8).
      # `Entry.in_pool_named` resolved `COALESCE(entries.pool_id, categories.pool_id)`; neither
      # column exists, so the field it backed is deleted from `Entry`'s `searchable` list.
  ...
  # ── THE WHOLE `"search by savings pool"` BLOCK IS DELETED (two-ledger spec §5, Task 8), and with
  # it the seven examples that were the only readers of `Entry.in_pool_named`. It searched the lane
  # `COALESCE(entries.pool_id, categories.pool_id)` resolved — including the pair that proved an
  # entry naming its OWN pool was found under that pool and not under its category's, which was the
  # defect the scope was written to fix. Both columns are dropped and the lane does not exist: a
  # category holds its own money and spending it never lands anywhere else.
  #
  # `type: :scope` ITSELF IS NOT DELETED. It is documented in `docs/searchable-system-reference.md`
  # with a note that it now has no live caller, because the SHAPE recurs the moment a searchable
  # lane is an expression rather than a foreign key.
