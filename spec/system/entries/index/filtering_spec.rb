...
RSpec.describe "Entries Index - Filtering", type: :system do
  ...
  describe "type tab filtering", :aggregate_failures do
    ...
    # A stale `?type=savings` bookmark falls through the filter's `else` to the unfiltered scope,
    # which is the All tab's own answer rather than an empty list under a tab that is not there.
