...
RSpec.describe "Sacrifices" do
  ...
  it "writes both cuts and lands on Budget once they close the gap", :aggregate_failures do
    patch_cuts(
      groceries_rule.id => { on: "1", amount: "700" },
      fun_rule.id => { on: "1", amount: "50" }
    )
  ...
  it "refuses a cut that does not undercut the claim, and writes nothing", :aggregate_failures do
    patch_cuts(groceries_rule.id => { on: "1", amount: "800" })
